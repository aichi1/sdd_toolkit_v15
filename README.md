# SDD Toolkit v15 — Spec-Driven Development for Claude Code
...
```bash
python3 -m pytest scripts/ -q          # 292 passed
python3 scripts/metrics.py --markdown  # 実測値の表。数値は手で書かずここから引く
```
...
│   ├── rules-reference/          rules の詳細版（自動注入されない。8 ファイル）
...
4. **Validator** が Quality Criteria の全項目を Met / Partial / Missing で判定し、`.validation/report.md` を作る
5. PASS → `outputs/.phase-context.json` を更新して次フェーズへ

複数フェーズ:

| 指定 | 戦略 |
|------|------|
| `/run-phase all` | スマートモード（既定）。ファストパスが効くフェーズは高速通過、問題があるフェーズだけ停止 |
...
| `spec_check.py` が意図的な参照を欠陥と報告する | `docs/spec-check-allowlist.json` に 1 件足し、**reason を必ず書く**（空だと無効）。前方参照は実物を作った時点で削除する |
...
```bash
python3 scripts/check_constitution.py --phase N   # 条文ごとの pass/fail/skip
```
...
### 5.5 `knowledge_curator.py` の重複追記（C-57・未修正）

`scripts/knowledge_curator.py:216` が `~/.sdd-knowledge/candidates.jsonl` へ**無条件に追記**します。同種の欠陥は `scripts/extract_components.py` では修正済みですが、こちらは残っています。`/retrospective` を繰り返すと `candidates.jsonl` が膨らみます。

**回避策**: `python3 scripts/promote_candidates.py --kb-dir ~/.sdd-knowledge` を実行すると `compact_candidates_file()` が重複排除して書き戻します。
...
| イベント | スクリプト | 動作 |
|---------|-----------|------|
| PreToolUse（`outputs/` への Write / Edit） | `check-docs-exist.py` | `docs/_manifest.json` の `required_files` を検証し、不足があれば**警告**（ブロックしない） |
| PostToolUse（`outputs/` への Write） | `post-phase-complete.sh` | コンポーネント候補を知識ベースへ自動抽出 |
| PostToolUse（`.metadata.json` への Write） | `check-validation-exists.py` | `.validation/` の有無を確認し、不足があれば**警告** |
| Stop | `remind-finalize.py` | 成果物があるのに `/finalize` 未実行ならリマインド |
...
```
python3 -m pytest scripts/ -q     → 292 passed
python3 scripts/spec_check.py     → 26 件
python3 scripts/trace_check.py    → 要件 0 件 / 欠陥 0 件
```

**`spec_check.py` の 26 件は欠陥ではありません。** `.claude/skills/` や `docs/rules-reference/` が `docs/requirements.md`・`docs/constitution.md`・`outputs/.phase-context.json` など **`/init-task` がこれから作るファイル**を参照しているためです。`/init-task` を実行すると解消します。残ったものは `docs/spec-check-allowlist.json` に reason 付きで登録してください。

### 開発リポジトリとの関係

このディレクトリは、ツールキットを v12 → v15 に改良した自己改善プロジェクトから**ツールキット本体だけを取り出した**ものです。以下は開発リポジトリ側に残しています。
